...
- **`BikeSpeedApp.swift`** constructs `LocationManager` then `TripManager(locationManager:)` (order matters — trip tracking depends on location) and injects both plus `SettingsStore` as environment objects.
