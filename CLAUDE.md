...
- **`Models/`** — `TripState` (idle/running/paused), `MeasurementSystem` (metric/imperial + conversion/formatting helpers), `CompassDirection` (8-point compass derived from a GPS course angle), plus the trip-log types below.
...
- **`Services/TripSyncEngine.swift`** — protocol + `NoOpTripSyncEngine`. Not for iCloud (that's a `ModelConfiguration` flag in `TripModelContainer`), but for a *non*-CloudKit backend. Every mutation in `TripDataStack` already announces itself to it, and `TripDataStackTests` fails if a new mutation path forgets to.
...
- `GENERATE_INFOPLIST_FILE = YES` — usage-description and other Info.plist keys live as `INFOPLIST_KEY_*` build settings in `project.pbxproj`, plus `InfoPlist.xcstrings` for the localized location usage description. Only a curated set of keys has an `INFOPLIST_KEY_*` mapping — an unmapped one (e.g. `INFOPLIST_KEY_UIBackgroundModes`) is accepted and then silently ignored, so verify a new key in the built product's `Info.plist`. Keys without a mapping go in the partial `Info.plist` at the repo root (`INFOPLIST_FILE = Info.plist`, merged with the generated keys); it sits outside `BikeSpeed/` deliberately — inside the synchronized folder it would also be copied as a resource and the build fails with two Info.plists.
